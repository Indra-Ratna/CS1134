...
class BinarySearchTreeMap:
    ...
    def subtree_insert(self, key, value=None):
        ...
        if (self.is_empty()):
            ...
            if (key < parent.item.key):
                new_node.ranking = parent.ranking - 1
    ...
    def increment(self, subtree_node):
        if subtree_node is None:
            return
        self.increment(subtree_node.left)
        subtree_node.ranking += 1
        self.increment(subtree_node.right)
    ...
    def decrement(self, subtree_node):
        if subtree_node is None:
            return
        self.decrement(subtree_node.left)
        subtree_node.ranking -= 1
        self.decrement(subtree_node.right)
    ...
    def __iter__(self):
        for node in self.inorder():
            yield (node.item.key, node.item.value, node.ranking)
...
#Question 5
    ...
    def get_ith_smallest_helper(self, i, subtree_node):
        if i == subtree_node.ranking:
            return subtree_node.item.key
        if i < subtree_node.ranking:
            return self.get_ith_smallest_helper(i, subtree_node.left)
        else:
            return self.get_ith_smallest_helper(i, subtree_node.right)
