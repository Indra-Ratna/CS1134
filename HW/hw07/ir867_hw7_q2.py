...
class LinkedBinaryTree:
    ...
    def leaves_list(self):
        if self.root is None:
            return []
        return self.find_leaves(self.root)

    def find_leaves(self, curr_root):
        ...
        if curr_root.left is not None and curr_root.right is not None:
            left = self.find_leaves(curr_root.left)
            right = self.find_leaves(curr_root.right)
            return left + right
        elif curr_root.left is not None and curr_root.right is None:
            return self.find_leaves(curr_root.left)
        else:
            return self.find_leaves(curr_root.right)
